"""
Lambda → Course-Service Assignment Flow Test

Lambda → SQS → Course-Service → DB 전체 플로우 테스트
(Lambda가 SQS에 메시지 발행 후 Course-Service가 처리)
"""
...
class TestAssignmentFlow:
    ...
    @pytest.mark.usefixtures("clean_sqs_queue", "test_course")
    def test_assignment_create_flow(
        ...
        assert assignment_created, "[ERROR] Assignment가 60초 내에 생성되지 않음"
    ...
    @pytest.mark.usefixtures("clean_sqs_queue")
