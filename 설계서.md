...
2. **llm-analysis-queue**
   ...
   - Consumer: LLM Lambda
   - Message: 과제 설명 분석 요청 (할일 자동 생성)

3. **task-creation-queue**
   - Producer: LLM Lambda
   - Consumer: Schedule-Service
   - Message: AI 생성된 할일(Todo) 및 서브태스크 정보

4. **submission-events-queue**
   - Producer: Canvas-Sync-Lambda
   - Consumer: LLM Lambda
   - Message: 과제 제출물 검증 요청

5. **calendar-events-queue**
...
For Each Course (Leader 토큰 사용)
  ...
  New Assignment Detected?
    ...
    1. Send to SQS (assignment-events-queue)
       ...
       → Schedule-Service: 일정(Schedule) 자동 생성

    2. Send to SQS (llm-analysis-queue)
       → LLM Lambda: 과제 설명 분석
       → Send to SQS (task-creation-queue)
       → Schedule-Service: 할일(Todo) 및 서브태스크 생성
  ↓
  Submission Detected?
    ↓ Yes
    Send to SQS (submission-events-queue)
    → LLM Lambda: 제출물 검증
    → Schedule-Service: 일정/할일 상태 업데이트 (유효 시)
  ↓
...
---
핵심 플로우:
Canvas 과제 → 일정(Schedule) 자동 생성 → LLM 분석 → 할일(Todo) 자동 생성
```
...
- SQS: `task-creation-queue`로 전송 → Schedule-Service가 Todo 생성
